...
void VkPostprocess::SetActiveRenderTarget()
{
	...
	fb->GetRenderState()->SetRenderTarget(&buffers->PipelineImage[mCurrentPipelineImage], buffers->PipelineDepthStencil.View.get(), buffers->GetWidth(), buffers->GetHeight(), SceneColorFormat(), VK_SAMPLE_COUNT_1_BIT);
}
